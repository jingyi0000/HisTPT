...
def build_eval_dataloader(cfg, ):
    ...
    for dataset_name in cfg['DATASETS']['TEST']:
        ...
        elif dataset_name == 'cityscapes_val_sem_seg': #add for cityscapes
            mapper = CityscapesSemDatasetMapper(cfg, False) 
            # mapper = MaskFormerSemanticDatasetMapper(cfg, False)
        # elif dataset_name == 'cityscapes_instance_seg_val'"
            # mapper = CityscapesInsDatasetMapper(cfg, False)
        elif dataset_name == 'ade20k_val_sem_seg':
            mapper = ADESemDatasetMapper(cfg, False)
        elif dataset_name in ['fog_acdc_val_sem_seg','night_acdc_val_sem_seg','rain_acdc_val_sem_seg','snow_acdc_val_sem_seg']:
            mapper = ACDCSemDatasetMapper(cfg, False)
        elif dataset_name == 'pascalcontext_val_sem_seg':
            mapper = PascalContextSemDatasetMapper(cfg, False)
        elif dataset_name == 'mapillary_vistas_sem_seg_val':
            mapper = MapillarySemDatasetMapper(cfg, False)
...
def build_train_dataloader(cfg, ):
    ...
    for dataset_name in dataset_names:
        ...
        # coco panoptic segmentation lsj new baseline
            ...
            loaders['coco'] = build_detection_train_loader(cfg, dataset_name=dataset_name, mapper=mapper)
        elif mapper_name == "cityscapes_sem_seg": #add for cityscapes
            mapper = MaskFormerSemanticDatasetMapper(cfg, True)
            loaders['coco'] = build_detection_train_loader(cfg, dataset_name=dataset_name, mapper=mapper)
        else:
            mapper = None
...
def build_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type == "cityscapes_ins_box":
        evaluator_list.append(InstanceSegEvaluator(dataset_name, output_dir=output_folder))
