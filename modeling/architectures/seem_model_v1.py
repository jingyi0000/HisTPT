...
from ..modules.point_features import (
...
class GeneralizedSEEM(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        # update weight_dict for new losses
        # weight_dict.update({"loss_entropy": 0.0})
        weight_dict.update({"loss_histpt": 1.0})
    ...
    def forward(self, batched_inputs, method='seem', mode='default'):
        """
        Args:
        ...
        if self.training:
            ...
            if method == "seem":
                ...
                losses_coop = self.forward_seg_coop(batched_inputs)
                losses.update(losses_coop)
    ...
    def forward_seg_coop(self, batched_inputs):

        n_cls = 20 #change for different datasets
        ...
        images_ori = [x["image_ori"].to(self.device) for x in batched_inputs]
        images_ori = [(x - self.pixel_mean) / self.pixel_std for x in images_ori]
        images_ori = ImageList.from_tensors(images_ori, self.size_divisibility)
        ##############

        # self.sem_seg_head.predictor.lang_encoder.get_text_embeddings(self.train_class_names, is_eval=False) #default
        self.sem_seg_head.predictor.lang_encoder.get_text_embeddings_coop(self.train_class_names, is_eval=False) #coop
        ...
        # for ori image
        with torch.no_grad():
            features_ori = self.backbone(images_ori.tensor)

        mask_features_ori, _, multi_scale_features_ori = self.sem_seg_head.pixel_decoder.forward_features(features_ori)

        # ####
        logits = outputs["pred_logits"].reshape(101,n_cls) #[1,101,20]

        #calculte entropy for update st memory 
        sample_logits = logits[0]
        sample_min = torch.finfo(sample_logits.dtype).min
        sample_logits = torch.clamp(logits, min=sample_min)
        sample_entropy = -(sample_logits * torch.exp(sample_logits)).sum(dim=-1)
        sample_entropy = sample_entropy.mean()

        
        ####### ST prompt update
        self.sem_seg_head.predictor.lang_encoder.update_st_memory(entropy=sample_entropy)
        self.sem_seg_head.predictor.lang_encoder.update_st_prompt()
        ####### Hard prompt update
        self.sem_seg_head.predictor.lang_encoder.update_hard_memory()
        self.sem_seg_head.predictor.lang_encoder.update_hard_prompt()
        ####### EMA prmopt update
        self.sem_seg_head.predictor.lang_encoder.update_ema_prompt()


        # for EMA prompt update
        outputs_global = self.sem_seg_head.predictor(multi_scale_features_ori, mask_features_ori, extra=extra, task='seg', ema=True)
        global_entropy = (outputs_global["pred_logits"].reshape(101,n_cls).softmax(1) * outputs_global["pred_logits"].reshape(101,n_cls).log_softmax(1)).sum(1)
        
        # for ST prompt update
        outputs_local = self.sem_seg_head.predictor(multi_scale_features_ori, mask_features_ori, extra=extra, task='seg', st=True)
        local_entropy = (outputs_local["pred_logits"].reshape(101,n_cls).softmax(1) * outputs_local["pred_logits"].reshape(101,n_cls).log_softmax(1)).sum(1)

        # for Hard prompt update
        outputs_hard = self.sem_seg_head.predictor(multi_scale_features_ori, mask_features_ori, extra=extra, task='seg', hard=True)
        hard_entropy = (outputs_hard["pred_logits"].reshape(101,n_cls).softmax(1) * outputs_hard["pred_logits"].reshape(101,n_cls).log_softmax(1)).sum(1)

        #weight calculation
        entropy_weight = F.softmax(torch.stack([global_entropy, local_entropy, hard_entropy], dim=-1), dim=-1) #[101,3]
        weight_global = entropy_weight[:,0].unsqueeze(1)
        weight_local = entropy_weight[:,1].unsqueeze(1)
        weight_hard = entropy_weight[:,2].unsqueeze(1)

        # fuse pseudo label
        softmax_fuse = F.softmax((weight_global*outputs_global["pred_logits"].reshape(101,n_cls)+weight_local*outputs_local["pred_logits"].reshape(101,n_cls)+weight_hard*outputs_hard["pred_logits"].reshape(101,n_cls)), dim=-1)
        conf_score_fuse, pseudo_label_fuse = softmax_fuse.max(-1)
        loss_fuse_logits = F.cross_entropy(logits, pseudo_label_fuse)

        losses = {}
        losses_tpt = {"loss_histpt": loss_fuse_logits}
        losses.update(losses_tpt)


        del outputs
        return losses

    def compute_ema_mask_loss(self, mask, mask_ema): #[n,128,256]
        num_mask = mask.shape[0]
        mask = mask[:, None]
        mask_ema = mask_ema[:, None] #[n,1,128,256]
        #sample point
        with torch.no_grad():
            # sample point_coords
        ...
        point_logits_ema = point_sample(
            mask_ema,
            point_coords,
            align_corners=False,
        ).squeeze(1)

        mask_dice_loss = dice_loss(point_logits.sigmoid(), point_logits_ema.sigmoid(), num_mask)
        mask_ce_loss = sigmoid_ce_loss(point_logits.sigmoid(), point_logits_ema.sigmoid(), num_mask)
        mask_loss = mask_dice_loss + mask_ce_loss
    ...
    def evaluate(self, batched_inputs):
        ...
        mask_features, _, multi_scale_features = self.sem_seg_head.pixel_decoder.forward_features(features)
        outputs = self.sem_seg_head.predictor(multi_scale_features, mask_features, task='seg')
