...
class SEEMDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg', extra={}, ema=False, st=False, hard=False):
        ...
        if ema: #add for ema output
            results = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0],ema=True)
        elif st: # add for st output
            results = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0],st=True)
        elif hard:
            results = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0],hard=True)
        else:
        ...
        for i in range(self.num_layers):
            ...
            output, query_embed = self.attention_data.cross_attn_variables()
            if ema: # add for ema
                results = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels], layer_id=i, ema=True)
            elif st: # add for st output
                results = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels], layer_id=i, st=True)
            elif hard:
                results = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels], layer_id=i, hard=True)
            else:
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, layer_id=-1, ema=False, st=False, hard=False):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        class_embed = decoder_output @ self.class_embed
        if ema:
            outputs_class = self.lang_encoder.compute_similarity(class_embed, ema=True)
        elif st:
            outputs_class = self.lang_encoder.compute_similarity(class_embed, st=True)
        elif hard:
            outputs_class = self.lang_encoder.compute_similarity(class_embed, hard=True)
        else:
