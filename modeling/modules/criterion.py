...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, layer_id, extra):
        ...
        src_masks = outputs["pred_masks"] #[bs, 101, 128, 256]
        src_masks = src_masks[src_idx] #[n, 128, 256]
        masks = [t["masks"] for t in targets]
        # TODO use valid to mask invalid areas due to padding in loss
        target_masks, valid = nested_tensor_from_tensor_list(masks).decompose()
        target_masks = target_masks.to(src_masks) #[bs, n, 512, 1024]
        target_masks = target_masks[tgt_idx] #[n, 512, 1024]
        # No need to upsample predictions as we are using normalized coordinates :)
        # N x 1 x H x W
        src_masks = src_masks[:, None] #[n,1,128,256]
        ...
        with torch.no_grad():
            ...
            ).type(src_masks.dtype) #[n, num_point, 2]
            # get gt labels
            point_labels = point_sample(
                target_masks,
                point_coords,
                align_corners=False,
            ).squeeze(1) #[n, num_point]
