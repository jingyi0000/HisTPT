...
class XDecoderPipeline:
    ...
    @staticmethod
    def forward_func(trainer, batch, method): #add method options
        loss = trainer.models['default'](batch, method=method)
        return loss
    ...
    def forward_step(
        ...
        loss = trainer.compute_loss(self.forward_func, batch, method) #add method options
    ...
    def evaluate_model(
        ...
        for dataset_label in dataset_names:
            ...
            with torch.no_grad():
                ...
                # model.model.sem_seg_head.predictor.lang_encoder.get_text_embeddings_coop(names, is_eval=True)
