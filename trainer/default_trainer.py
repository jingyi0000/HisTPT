...
class DefaultTrainer(UtilsTrainer, DistributedTrainer):
    ...
    def __init__(self, opt):
        ...
        self.test_time_evaluator = build_evaluator(self.opt, self.opt['DATASETS']['TEST'][0], self.opt['SAVE_DIR'])
    ...
    def _eval_on_batch(self, batch):
        logger.info(f"Test Time Evaluation ...")
        if self.opt['FP16']:
            from torch.cuda.amp import autocast
            with autocast():
                batch_results = self.pipeline.test_time_evaluate_model(self, batch)
        else:        
            batch_results = self.pipeline.test_time_evaluate_model(self, batch)
        # if self.opt['rank'] == 0:
    ...
    def compute_loss(self, forward_func, batch, method): #add method options

        def forward(func, trainer, batch, method):
            if self.opt['FP16']:
                from torch.cuda.amp import autocast
                with autocast():
                    loss = func(trainer, batch, method)
            else:
                loss = func(trainer, batch, method)
            return loss

        loss = forward(forward_func, self, batch, method)
        return loss
    ...
    def train_step(self, batch, method="seem"): #add method options
    ...
    def init_train(self):
        ...
        self.raw_models = self.pipeline.initialize_model()
        self.model_names = list(self.raw_models.keys()) # ['default']
    ...
    def train(self, method="seem"): #add method options
        """
        Training
        """
        self.init_train()
        ...
        for epoch in range(self.train_params['start_epoch_idx'], num_epochs):
            ...
            self.test_time_evaluator.reset()
            for batch_idx, batch in enumerate(self.train_dataloaders):
                ...
                self.train_step(batch, method) #add method options
                ...
                for b in batch:
                    new_dict = dict()
                    new_dict.update({"image": b["image_eval"]})
                    new_dict.update({"semseg": b["semseg_eval"]})
                    new_dict.update({"width": b["width"]})
                    new_dict.update({"height": b["height"]})
                    new_dict.update({"file_name": b["file_name"]})
                    batch_eval.append(new_dict)


                batch_results = self._eval_on_batch(batch_eval)


                self.test_time_evaluator.process(batch_eval, batch_results)

                if batch_idx + 1 == self.train_params['updates_per_epoch']:
                    results = self.test_time_evaluator.evaluate()
                    print(results)
